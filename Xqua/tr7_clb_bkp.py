...
DATASETS = [
    ...
    r"C:\Users\LTSS2023\Documents\elhadjmb\datasets\blc_4",
    r"C:\Users\LTSS2023\Documents\elhadjmb\datasets\blc_2",
    r"C:\Users\LTSS2023\Documents\elhadjmb\datasets\blc_3",
    r"C:\Users\LTSS2023\Documents\elhadjmb\datasets\blc_7",
    r"C:\Users\LTSS2023\Documents\elhadjmb\datasets\blc_8",
...
def get_transform(coco_path=None, mean=(0.613873742152602, 0.5445290026996225, 0.5178894349877393),
                  std=(0.1441995606320819, 0.1583217423812118, 0.17013199527272097)):
...
def start(coco_json_path, num_classes=5, train_val_split=0.8, resume_from_checkpoint=None, batch_size=1,
    ...
    # Getting transforms...
    transform = get_transform(coco_path=coco_json_path)
...
def objective(trial):
    ...
    batch_size = trial.suggest_categorical('batch_size', [1, 2, 4])  # 4, 8, 16
    learning_rate = trial.suggest_loguniform('learning_rate', 1e-4, 1e-1)
    momentum = trial.suggest_uniform('momentum', 0.75, 0.8)
    weight_decay = trial.suggest_loguniform('weight_decay', 1e-5, 1e-3)
    lr_scheduler_factor = trial.suggest_uniform(
        'lr_scheduler_factor', 0.2, 0.9)
    lr_scheduler_patience = trial.suggest_int('lr_scheduler_patience', 3, 8)
    score_thresh = trial.suggest_uniform('score_thresh', 0.3, 0.6)
    num_epochs = trial.suggest_int('num_epochs', 50, 300)  # 50-300
    train_val_split = trial.suggest_uniform('train_val_split', 0.7, 0.8)
    model_name = trial.suggest_categorical(
        'model_name', ['resnet50', 'resnet101', 'resnet152'])
    anchor_size = trial.suggest_categorical('anchor_size',
                                            [((16,), (32,), (64,), (128,), (256,)), ((8,), (16,), (32,), (64,), (128,)),
                                             ((
                                                  32,), (64,), (128,), (256,), (512,)),
                                             ((64,), (128,), (256,), (512,), (1024,)),
                                             ((64,), (128,), (256,), (512,), (1024,)),
                                             ((64,), (128,), (256,), (512,), (1024,))])
    anchor_ratio = trial.suggest_categorical('anchor_ratio',
                                             [(0.5, 1.0, 2.0), (1.0, 1.0, 1.0), (0.5, 1.0, 1.5), (1.0, 2.0, 3.0)])
    # coco_dataset = trial.suggest_categorical('coco_dataset', [r"C:\Users\LTSS2023\Documents\elhadjmb\eds\blc\annotations.json", r"C:\Users\LTSS2023\Documents\elhadjmb\fds\blc\annotations.json"])
    coco_dataset = DATASETS_MC['3']
    try:
        curr_map = start(
            coco_dataset,
            batch_size=batch_size,
        ...
        print(e)
        print("Something happend! continuing anyway...\n\n\n")
        curr_map = 0

    return curr_map  # Optuna minimizes this return value
...
def level(number, dataset, model, single_cat):
    ...
    batch_size = 2 if model == "resnet152" else 4 if model == "resnet101" else 8
    ...
    train_val_split = 0.6
    anchor_size = ((28,), (54,), (80,), (106,), (132,))
    # ((10,), (26,), (42,), (57,), (73,)) for blc7  # ((16,), (32,), (48,), (64,), (80,))
    anchor_ratio = (0.5998266656490321, 0.9571966923446008, 1.3145667190401695)
...
def main():
    ...
    while curr_map < 0.9 or i == max_iters:
        msg("Starting optimizer")

        study.optimize(objective, n_trials=max_iters)

        # get best params
        best_params = study.best_params
        save_hparam(r"C:\Users\LTSS2023\Documents\elhadjmb" + "/progress/optuna.json", best_params)
        # update curr_map
        curr_map = study.best_value
        i += 1

        msg(f"Current iteration: {i}")
        msg(f"Current min MAGIC: {curr_map:.2f}")
    msg("All done. Congratulations!")


if __name__ == '__main__':
    # for number in range(1, 4 +1):
    #     for idx, dataset in enumerate(DATASETS[:]):
    #         single_cat = True if (number <= 2) else False
    #         if (single_cat and idx == 2) or (not single_cat and idx == 0):
    #             continue
    #         for model in MODELS:
    ...
    for number in range(1, 2 + 1):
        dataset = DATASETS[5]
        single_cat = True if (number == 1) else False
        model = "resnet152"
        level(number=number, model=model, dataset=dataset, single_cat=single_cat)
